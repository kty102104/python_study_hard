...
'''
    1. if문
        if문은 조건이 참(True)일 때만 해당 블록의 코드를 실행한다.
...
'''
    if문에서 신경써야 할 점 :
        1) if와 같은 라인에 작성된 코드는 참/거짓으로 구분할 수 있어야 함.
...
#     print("성인이 아닙니다.")
'''
    이상의 코드에서 21번 라인의 print문은 if절에, 
    23번 라인의 print문은 else문에 종속되어 있다는 점을
    들여쓰기를 통해 알아볼 수 있다.
...
19 미만이라면 곧장 72번 실행문이 실행됩니다.
만약에 19 이상이라면, 티켓 유무에 따라서 67번 또는 69번 라인이 실행
'''
...
'''
    5. 조건문에서 자주 쓰이는 연산자
        ...
        논리 연산자 :
            ...
            2) or : A or B -> A와 B 둘중 하나가 참일 때 실행문 실행
