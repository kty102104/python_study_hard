...
# len() : 문자열 혹은 collection의 총 개수를 int 형태로 반환하는 함수 -> hangman01 확인

#TODO - 1 : 빈 list인 display를 만드세요.
display = []

#TODO - 2 : chosen_word의 문자 개수만큼 display에 "_"를 추가하세요.
# 예를 들어서 chosen_word = camel이라면 display = [ "_", "_", "_", "_", "_" ]가 돼야 합니다.
# 여기에 display.append()를 이용해야 하고, chosen_word의 문자 개수만큼 반복이 이루어져야 합니다.

# TODO - 2를 해결하기 위해서 알아야 하는 list 관련 method :
# 리스트명.append() : 리스트의 맨 마지막인덱스에 () 안의 요소를 추가해주는 method

word_list.append("dictionary")
# print(word_list)

for _ in range(len(chosen_word)):   # 강사가 어떤 의미로 i 혹은 _를 넣는지를 구분하는 이유
...
# ex) chosen_word = apple인데, guess = p라면, display = [ "_", "p", "p", "_", "_" ]가 돼야 합니다.
