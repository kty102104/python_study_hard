...
def play_hangman():
    ...
    # random.choice()를 이용하여 임의의 단어 추출
    ...
    while not end_of_game:
        ...
        print(" ".join(display))
            ...
            # print(" ".join(display))
        # 반복문 바깥에서 guess가 chosen_word에서 하나도 일치하지 않는다면 lives -= 1 해야함 -> 행맨 그림도 필요
        if guess not in chosen_word:
            lives -= 1

            #lives가 변하는지 확인하는 테스트 코드
            # print(f"테스트 코드 : {lives}")
