...
# list_num2 = list([4, 5, 6, 7, 8, 9])        # list 생성 방법 2(형변환 함수 사용)
# print(list_num1)
...
# print(scores)           # collection의 이름은 복수형이 기본
#
# scores.insert(0,90)
# print(scores)
'''
...
'''
# se2 = set({})           # 비어있는 세트 생성 방법 1
# print(type(se2))
# se3 = set()             # 비어있는 세트 생성 방법 2
# print(type(se3))
'''
...
# print(set_num5)                 # 중복 제거 된 것을 확인할 수 있음.  {1, 2, 3}
# list_num6 = list(set_num5)      # set()으로 중복 제거 후, list()를 이용하여 인덱스 / 마이너스 인덱스/ 슬라이싱을 재이용함.

'''
    set에는 인덱싱/ 마이너스 인덱싱/ 슬라이싱을 지원하지 않기 때문에 특정 요소만 추출하기 위해서는 형변환하는 과정(주로 list)이
        필요함
...
# set_num6 = {10, 20, 30}
# set_num6.add(50)
# print(set_num6)     # 순서가 없기 때문에 출력 결과가 순서대로 나오지 않을 수 없음
...
#
# print(type(dict_num1.keys()))           # 이것을 확인해보면 얘네는 list가 아니기 대문에 인덱스가 없습니다.
...
# sliced_list = list_original[2:7:]
# print(f"3번째 요소로부터 7번째 요소 = {sliced_list}")
# print(f"3번째 요소로부터 7번째 요소 중 2번째 요소 = {sliced_list[1]}")

'''
사용자로부터 1에서 12사이의 월을 입력 받아, 해당 월이 며칠까지 있는지 출력하는 프로그램을 작성하세요.
...
#
# print(f"{month}월은 {last_date_list[month-1]}일까지 있습니다.")


# last_date_short = [28, 31, 30]
# month = int(input("1 ~ 12 사이의 월을 입력하세요 >>> "))
#
# if month == 2:
...
#
# for _ in range(3):
#     field_trip_set.add(input("희망하는 수학여행지를 입력하세요 >>> "))
#
...
#
# for i in range(1,num+1):                    # 시작값, 종료값, 증감값을 이용한 방식
#     basket.append(input(f"{i}번째 과일을 입력하세요 >>> "))
#
...
#
# for i in range(int(input("몇 개의 숫자를 입력할까요? >>> "))):
#     all_numbers.append(int(input(f"{i+1}번째 숫자를 입력하세요 >>> ")))
#
# # 향상된 for문 이용
...
#     telephones[name] = input(f"{i}번째 사람의 전화번호를 입력하세요 >>> ")
#
...
list[] : append, insert(인덱스, 값) / pop(인덱스), remove(값)
-slicing
tuple() : X
-slicing
