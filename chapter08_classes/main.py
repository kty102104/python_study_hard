...
함수를 처리할 필요가 있습니다.
'''
# student_info() 함수를 호출하고, 내부에 여러분의 프로필을 작성하세요.
...
클래스를 작성하는 것은 클래스 정의라고 합니다 -> 함수 정의
...
'''
# 클래스 정의 형식 예시
# class WaffleMachine:        # 클래스명은 대문자로 작성하고, upper camel case, Pascal case 변수는 snake_case
...
# print(waffle)
'''
print(waffle)을 실행시켰을 때 <__main__.WaffleMachine object at 0x0000021669794E50>에서 object라고 표기된 점을
...
1. 클래스의 기본 구성
    ...
    객체를 생성하기 위해서 클래스는 객체가 가져야 할 '값'과 '기능'을 가져야 합니다.
...
#
# # 속성에 값 대입
# print(pokemon2.number)      # pokemon2 객체의 속성인 number 값을 조회
# pokemon2.number = 5         # 객체명.속성명 = 데이터         -> 변수 대입하는 것과 같은 방식으로 바꿀 수 있음
#
# # 한 개체의 속성을 직접 조회하는 방식
...
- 인스턴스 변수는 객체의 데이터(값)을 저장합니다. 예를 들어 pokemon1 객체의 인스턴스 변수는 "이상해씨"라는 값을 가짐.
...
#객체 생성
# person1 = Person() # 생성자를 클래스에 정의하지 않았기 때문에 pokemon3과 달리 소괄호 내에 아무런 값이 없어도 생성 가능
#
# # 객체에 값 대입
...
'''
# person1.set_info("김일", 20, "010-1234-4567", "뉴욕주 뉴욕시")
# print(f"제 이름은 {person1.name}입니다.\n나이는 {person1.age}살이구요\n연락처는 {person1.tel}입니다."
#       f"\n{person1.address}에 살고 있습니다.")
#
# person1.set_name("김일")
# person1.set_age(20)
# person1.set_tel("010-1234-4567")
# person1.set_address("뉴욕주 뉴욕시")
# print(f"제 이름은 {person1.get_name()}입니다.\n나이는 {person1.get_age()}살이구요\n연락처는 {person1.get_tel()}입니다."
