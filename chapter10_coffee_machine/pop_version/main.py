...
def is_resource_enough(order_ingredients):
    ...
    for item in order_ingredients:
        if order_ingredients[item] > resources[item]:
            ...
            return False
    return True # 굳이 else를 쓰지 않은 이유 : 위의 if문이 실행되지 않았다면 어차피 True기 때문
...
while is_on:
    ...
    if choice == "off":
        is_on = False
        print("자판기를 종료합니다.🙌")
    # 만약에 choice가 report라면 현재의 물, 우유, 커피, 그리고 돈을 조회할
    # 수 있는 코드를 작성하세요.
    if choice == "report":
    ...
    #여기에 입력할 조건문 : choice가 라떼, 에스프레소, 카푸치노 중 하나일 경우를 elif로 작성하시오.
        ...
        if is_resource_enough(MENU[choice]["재료"]):
            if is_transaction_successful(process_coins(), MENU[choice]["가격"]):
