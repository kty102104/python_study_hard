...
#     country = "한국"      # 그냥 다짜고자 선언해서 초기화하면 클래스 변수
#
...
#     def trip(cls, traveling_site):  # 클래스 메서드에서 클래스 변수를 참조할 때는 cls를 명시
#         if cls.country == traveling_site:
#             print("국내 여행입니다.")
#         else:
#             print("해외 여행입니다.")
#
...
정적 메서드는 self, cls를 다 사용하지 않기 대문에 인스턴스/클래스 변수를 모두 사용하지 않는 메서드를
...
class Shop:
    ...
    # @staticmethod
    # def find_menu(menu):
    #     for menu_dict in Shop.menu_list:
    ...
    #     print(f"{menu}을(를) {quantity}개 판매")
    #     cls.total += cls.find_menu(menu) * quantity

    @classmethod
    def sales(cls, menu, quantity):
        for menu_dict in cls.menu_list:
            # 여기서 또 for문 돌려서 키와 값을 추출 가능 -> 하지만 menu_list의 형태를 보니 적절치 않다.
            if menu in menu_dict:    # menu_dict에서 조건문을 걸었는데, 기본적으로 key를 중심으로 탐색합니다.
                # print(f"메뉴 : {menu}\n가격 : {menu_dict[menu]}개 판매")
