'''
1. 파일 입출력의 개요
    파이썬을 이용하여 컴퓨터에 저장된 파일을 읽어들이는 것과, 새로운 파일을 생성하여
    컴퓨터에 저장하는 것이 가능
...
# open(".sample.txt")  # 현재 디렉토리(.)를 기준으로 경로를 설정하는 방법
...
+------+------+-----------+-------------+---------------------+---------------------+
                입출력 모드를 생략하면 기본적으로 r모드로 파일이 열립니다.
...
#
# print(str, end="🙌")  # keyword argument 형태로 end 속성을 불러왔고,
#                     # 거기에 ""이라는 데이터를 대입
# file.close()
'''
...
#     if not str:
#         break
#     print(str, end="🙌")              # 이 경우에는 str에 문자 하나씩 대입하고 출력하는 것을 반복합니다. 그렇기 대문에 end=""이 필수
# file.close()
'''
            2) readline() 메서드
